...
/**
 * Class for a more flexible TreeNode than DefaultMutableTreeNode. The class is programmed as a generic working on IGuifiable objects
 *
 * @param <T> Type variable for the type of the user object.
 */
@SuppressWarnings("serial")
public class FlexibleTreeNode<T extends IGuifiable> extends DefaultMutableTreeNode
        implements IDebugable{
    ...
    public FlexibleTreeNode(T userObj){
        ...
        // but it's quite ok.
        ErrorHandler.Assert(userObj != null, true, FlexibleTreeNode.class,
                "No user object provided!");
    }

    @SuppressWarnings("unchecked")
    public List<FlexibleTreeNode<T>> getLeaves(){
        List<FlexibleTreeNode<T>> leaves = new ArrayList<>(100);
        if(this.isLeaf()){
            leaves.add(this);
        }else{
...
//        return list;
    ...
    public void getAllUserObjects_recursive(List<T> lstResults,
            FlexibleTreeNode<T> tnCurrent){
        lstResults.add(tnCurrent.getUserObject());

        for(int i = 0; i < tnCurrent.getChildCount(); i++){
            @SuppressWarnings("unchecked")
            FlexibleTreeNode<T> tnChild = (FlexibleTreeNode<T>) tnCurrent
                                .getChildAt(i);
    ...
    public String toDebugString(){
        try{
            ...
            return "Unable to fully resolve " + this.getClass().getName() + " [" + this
                    .toString() + "]";
        }
    }

    /**
     * Overwritten toString(). The method now calls toGuiString of the generic object to do easier working with the IGuifiable-Pattern. As this method is used by JTree.
     *
    ...
    /**
     * Gets all tree nodes of the tree defined by tnStart (including tnStart!) as a list HINWEIS: Diese Methode könnte man eigentlich auch static machen, weil das eigentliche
     * Knotenobjekt gar nicht verwendet wird. Ich habe darauf verzichtet, weil wegen des generischen-Typparameters hier sonst Probleme auftreten, die Sie noch mehr verwirren
     * würden. -> Sehen Sie hierzu aber die danach auskommentierte Methode, die zeigt, wie man Sie als static-Methode deklarieren müsste - hauptsächlich müsste man dann einen neuen
     ...
     */
    public /* static */ List<FlexibleTreeNode<T>> getAllTreeNodesAsList(
                    FlexibleTreeNode<T> tnStart){
        ...
        List<FlexibleTreeNode<T>> lstRet = new ArrayList<>(100); // Ineffizient!!!!!

        lstRet.add(tnStart);

        for(int i = 0; i < tnStart.getChildCount(); i++){
            @SuppressWarnings("unchecked")
            FlexibleTreeNode<T> tnChild = (FlexibleTreeNode<T>) tnStart
                                .getChildAt(i);
    ...
    /**
     * Effizientere Variante von getAllTreeNodesAsList, die nur einmal eine Liste erzeugt und mit this arbeitet.
     *
     * @return list with the tree nodes.
     */
    public List<FlexibleTreeNode<T>> getAllTreeNodesAsListMoreEfficient(){

        List<FlexibleTreeNode<T>> lstResults = new ArrayList<>(countTreeNodes(
                                  this) + 5);
    ...
    /**
     * Gets all tree nodes of the tree defined by tnStart (excluding tnStart!) as List. HINWEIS: Diese Methode könnte man eigentlich auch static machen, weil das eigentliche
     * Knotenobjekt gar nicht verwendet wird. Ich habe darauf verzichtet, weil wegen des generischen-Typparameters hier sonst Probleme auftreten, die Sie noch mehr verwirren
     * würden.
     *
     * @param tnStart IN: The tree node to start
     ...
     * @return list with the tree nodes.
     */
    public /* static */ List<FlexibleTreeNode<T>> getAllSubTreeNodesAsList(
                    FlexibleTreeNode<T> tnStart){

        List<FlexibleTreeNode<T>> lstRet = new ArrayList<>(100);
    ...
    @SuppressWarnings("unchecked")
    private void toDebugString_recursive(StringBuilder sb, int indentLvl,
            FlexibleTreeNode<T> tn){
        toDebugString_indent(sb, indentLvl);
        sb.append(tn.toString());
        sb.append("\n");
        for(TreeNode tnChild : tn.getChildren()){
            toDebugString_recursive(sb, indentLvl + 1,
                    (FlexibleTreeNode<T>) tnChild);
        }
    }
    ...
    * lstToReturn.addAll(getAllTreeNodesAsList(tnChild)); } return lstToReturn; }
     */
    private void getAllTreeNodesAsListMoreEfficient_recursive(
            List<FlexibleTreeNode<T>> lstResults,
            FlexibleTreeNode<T> tnCurrent){
    ...
    /**
     * Gets all tree nodes of the tree defined by tnStart (including tnStart!) as List NOTE: Same as getAllTreeNodesAsList, but more efficient because we first create a list and
     * then use it in the recursion HINWEIS: Diese Methode könnte man eigentlich auch static machen, weil das eigentliche Knotenobjekt gar nicht verwendet wird.Ich habe darauf
     * verzichtet, weil wegen des generischen-Typparameters hier sonst Probleme auftreten, die Sie noch mehr verwirren würden.
     *
     * @param <Z>
     * @param tnStart IN: The tree node to start
     *
    ...
    public static <Z extends IGuifiable> List<FlexibleTreeNode<Z>> getAllTreeNodesAsListMoreEfficient_old(
        ...
        List<FlexibleTreeNode<Z>> lstToReturn = new ArrayList<>(10);
        if(tnStart == null){
            return lstToReturn;
        }
    ...
    private static <Z extends IGuifiable> void getAllTreeNodesAsListMoreEfficient_recursive(
            FlexibleTreeNode<Z> tnStart,
            List<FlexibleTreeNode<Z>> lstToReturn){
        lstToReturn.add(tnStart);
        for(int i = 0; i < tnStart.getChildCount(); i++){
            @SuppressWarnings("unchecked")
            FlexibleTreeNode<Z> tnChild = (FlexibleTreeNode<Z>) tnStart
                                .getChildAt(i);
